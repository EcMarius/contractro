<?php
	...
	// Get contracts for the invoice's company
...
<x-layouts.app>
	<x-app.container class="max-w-5xl space-y-6">
		...
		{{-- Header --}}
		<div>
			<a href="{{ route('invoices.show', $invoice->id) }}" class="inline-flex items-center text-sm text-gray-600 hover:text-gray-900 dark:text-gray-400 dark:hover:text-gray-100 mb-4">
		...
		{{-- Form --}}
		<form method="POST" action="{{ route('invoices.update', $invoice->id) }}" class="space-y-6" x-data="invoiceForm()">
			@csrf
			@method('PUT')
